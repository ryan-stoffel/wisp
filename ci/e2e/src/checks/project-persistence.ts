...
import { TIMEOUT_MS, launchConnected, projectWorkspace, ready, type Session } from '../harness.ts';
...
export const projectPersistenceChecks = [
  check('creating a project shows it in the sidebar and it survives a relaunch', async () => {
    ...
    try {
      ...
      const created = await projectRowLabels(session.window);
      if (created.length !== 1) {
      ...
      const afterRelaunch = await projectRowLabels(session.window);
