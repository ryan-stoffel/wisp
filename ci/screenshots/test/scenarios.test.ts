...
test('scenarios with their own environment put the fake claude first on PATH', () => {
  for (const scenario of scenarios.filter((candidate) => candidate.env)) {
    const path = scenario.env?.('/unused').PATH ?? '';
    assert.match(path.split(delimiter)[0] ?? '', /ci\/smoke\/fixtures\/fake-cli$/, scenario.name);
  }
});
