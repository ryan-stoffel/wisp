...
- **Done** means you accepted the agent's changes: they are merged into the project's branch on the host, and the agent's worktree and branch are gone, so its row has no Review changes action and takes no more messages.
...
| Diff review | M3 | The editor area: a multi-file diff of the agent's worktree against its base, titled "Review: {agent}" | A review bar shows the agent, diff stat, test result, and host. **Accept** is primary; **Request changes** is secondary and puts the cursor in the composer with the agent named, and sending it goes to the agent as a follow-up. Accept (#68) merges the agent's branch into the project's current branch on its host, fast-forward when it can, otherwise with a merge commit, and never pushes; then it removes the agent's worktree and branch, and the agent shows Done. Accept's tooltip: **Merge {branch} into {current branch} on {host}. Nothing is pushed.** If uncommitted changes touch the same files, or the branch conflicts, nothing changes and the bar says so: **Commit or stash your changes to {files}, then accept again**, or **{agent}'s changes conflict with {current branch} in {files}. Request changes to have it update its branch.** In M4, when overlapping changes make a diff stale, the bar shows Conflicts with {agent} (#70). |
