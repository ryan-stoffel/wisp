...
- **A failed checkout is rolled back.** `merge --ff-only` can stop part way when one of those filters fails (a required LFS smudge that can't download) or runs past its 5-minute timeout. git has moved neither the branch nor the index then, only some files. wispd puts them back: it removes the files the merge added and checks out the rest from the index, `.gitattributes` first, then reports the true state. This is safe because the overlap check proved none of these paths had local changes. Refusing up front whenever `.gitattributes` changes was rejected: agents edit it legitimately, and a failing filter doesn't need an agent to be involved.
