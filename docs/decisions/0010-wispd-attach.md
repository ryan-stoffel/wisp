...
- attach then retries with backoff, starting at 10 ms and doubling up to 500 ms, until `--connect-timeout` passes.
  - The timeout is 10 s by default, which is the editor's liveness window (0007).
- attach never stops or signals a wispd, including one it started.

### Starting wispd

- **The LaunchAgent:**
  - Label: `io.github.ryan-stoffel.wisp.wispd`, which is 0006's bundle id plus `.wispd`.
  ...
  - If `kickstart` fails, attach says so on stderr and starts `serve` itself.
...
- **#61:** it installs the LaunchAgent under the label, plist path, and domain above, serving the default data folder, or it changes attach to match.
