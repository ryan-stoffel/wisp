...
use super::{
    BackendRegistry, Defaults, KeyAccounts, PolicyCheckError, RoutingError, check_no_write_policy,
    resolve, start,
};
use crate::backend::{
    AccountRef, Backend, CancelSwitch, Capabilities, Credential, EVENT_BUFFER, Event, EventSink,
    EventStream, Failure, FailureKind, Outcome, RunHandle, RunId, RunRequest, StartError, Started,
    ToolPolicy,
};
...
/// [`Backend::start`], in order, and records which account each call used. Unlike
/// [`crate::backend::fake::FakeBackend`], it spawns no process, so its scripted attempts can
/// differ from one call to the next: exactly what testing a fallback, which restarts the same
/// backend with a different credential, needs.
struct ScriptedBackend {
    attempts: Mutex<VecDeque<Vec<Event>>>,
    calls: Mutex<Vec<AccountRef>>,
}
...
impl Backend for ScriptedBackend {
    ...
    fn start(&self, request: RunRequest) -> Result<Started, StartError> {
        self.calls.lock().unwrap().push(request.account.clone());
        let events = self
            .attempts
            .lock()
            .unwrap()
        ...
        let (handle, _control) = RunHandle::new(request.run_id, false, CancelSwitch::new());
        Ok(Started {
            run: Arc::new(handle),
            events: stream,
        })
...
fn an_explicit_choice_is_used_even_when_the_roles_default_differs() {
    ...
    .unwrap();
    assert!(matches!(resolved.selection, super::Selection::Key { id } if id == key_id));
}
...
async fn a_signed_out_subscription_falls_back_once_to_the_configured_key_account() {
    ...
    assert_eq!(calls[1].id, fallback_id.to_string());
    let Credential::ApiKey(key) = &calls[1].credential else {
        panic!(
...
async fn a_clean_working_tree_has_no_violation() {
    ...
    assert_eq!(check_no_write_policy(dir.path()).await.unwrap(), None);
}
...
async fn a_change_to_a_tracked_file_is_a_violation() {
    ...
    let violation = check_no_write_policy(dir.path()).await.unwrap().unwrap();
...
async fn a_directory_that_is_not_a_git_repository_is_an_error() {
    let dir = tempfile::tempdir().unwrap();
    let error = check_no_write_policy(dir.path()).await.unwrap_err();
    assert!(
