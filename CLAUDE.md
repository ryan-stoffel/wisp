...
- Visuals only where they show what changed: a PR with a user-visible change gets the `screenshots` label and a `wisp-media` block (captures of a new view, a before and after of a bug fix, a video of a flow); backend, CI, docs, refactor, and test-only PRs get neither. The reviewer flags a UI PR without visuals and a non-UI PR with them.
...
- `screenshots.yml`: only PRs with the `screenshots` label ([0018](docs/decisions/0018-pr-visuals-on-request.md)). It runs when the label is added and on each push or description edit while it is on, and captures only the scenes that the hidden `<!-- wisp-media ... -->` block in the PR body names, from `ci/screenshots/src/scenarios.ts`: `after` for a new view, `before-after` for a user-visible bug fix (base app and head app side by side), `video` for a flow (a GIF preview linking the full video). Files are committed to the orphan branch `ci-screenshots` under `pr-<number>/<short-sha>/`, and the workflow writes them into the `## Screenshots` section of the PR body, between `<!-- wisp-media:start -->` and `<!-- wisp-media:end -->`, never as a comment; later pushes replace that section. Without the label nothing runs; with it, a missing block or an unknown scene fails the workflow.
