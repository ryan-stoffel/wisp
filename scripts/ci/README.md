...
- **Syntax.** One `mode: scene, scene` line per mode. Scenes are the `name`s in `ci/screenshots/src/scenarios.ts`, separated by commas or spaces. Blank lines and lines starting with `#` are skipped, and `;` also ends a line. Each scene appears at most once (`before-after` already includes the after), and a block names at most 10. The first block counts whose `<!-- wisp-media` opening is a line of its own, outside fenced code and outside the section the workflow writes, so a description can quote a block, or mention the markers, without triggering anything. `ci/screenshots/src/request.ts` parses it.
...
- `request` (Linux, `contents: read` and `pull-requests: read`) reads the PR from the API, so labels and body are current even for `workflow_dispatch`. Its first step, `Read the pull request`, is the gate that PR code cannot change: before anything from the PR is checked out, it stops a PR whose head is not in this repository (a notice, for every event, `workflow_dispatch` included), a PR without the label (a notice, or an error for `workflow_dispatch`), and a closed one (an error). This matters for `workflow_dispatch`, whose token can write even for a fork's PR number; on a `pull_request` event from a fork the token is read-only anyway. Past the gate, it checks out the head's `ci/screenshots/`, installs its one runtime dependency, and runs `src/resolve.ts`: it applies the rules above, checks every scene against `scenarios.ts`, and, for `before-after`, asks the compare API for the merge base of the base branch and the head. That commit is the "before", so the pair differs by this PR only. A labeled PR with no block, a malformed line, a duplicate, or an unknown scene fails this job with an `::error::` that says what to fix, and `publish` writes the same message into the section. `resolve.ts` checks the fork, label, and state again, as a second line behind the gate.
- `capture` (macOS, `contents: read`) gets the head's arm64 `Wisp.app`, and for `before-after` the base commit's too (see [Getting the app](#getting-the-app)). For a `video` it installs Homebrew's `ffmpeg`, since the runner image has none and Playwright's own build only writes WebM. Then it runs `screenshots <dir> --request <scenes>` and uploads the files, `manifest.json`, and the steps' logs as the `screenshots` artifact. `screenshots` installs `ci/screenshots/`, whose only runtime dependency is `playwright-core`, and runs each requested scene against a fresh launch of the app from `app-launch`'s output:
  ...
  - `video`: the scene with Playwright's `recordVideo` at the window's size, held on its last state for 2 seconds. It keeps the window's video as `<name>.webm`, failing only that scene when the WebM is over the 25 MB that `publish` accepts, and makes `<name>.gif` with an ffmpeg palette pass: 10 fps at 800 px wide, then 8 fps at 640 px, then 5 fps at 480 px, until the GIF is at most 10 MB. A scene that relaunches the app records its last launch. `screenshots` downloads Playwright's ffmpeg, which `recordVideo` encodes with.
...
- `publish` (Linux, `contents: write` and `pull-requests: write`) runs after `capture`, even when it failed, and after a `request` job that found a problem to report. It checks out only `scripts/ci/` and `ci/screenshots/src/`, installs nothing, downloads the artifact, and runs `publish-screenshots`. That commits the files to the orphan branch `ci-screenshots` under `pr-<number>/<short-sha>/`, then writes the section. It reads the PR body again just before its `PATCH`, replaces only the text from the `<!-- wisp-media:start -->` line through the `<!-- wisp-media:end -->` line, counting each marker only on a line of its own outside fenced code (or appends the section, the first time), and never posts a comment. It leaves the section as it is when the head has moved on (that commit's run writes it), when the label is gone, or when the request block differs from the one the `request` job read, which it tells by a SHA-256 of the block. That last check matters because a run whose `request` job finds a problem, or skips, never starts a `capture`, so it cannot cancel an older capture still running; without it, that capture's `publish` would overwrite the newer problem report with scenes nobody asks for anymore. It refuses, with an error and no write, a body with a start marker and no end marker, which would otherwise take the author's text after it, and a body that ends inside an unclosed code fence, where an appended section would be hidden and appended again on every run. Images are `raw.githubusercontent.com` URLs pinned to the `ci-screenshots` commit, so no cache shows an old image. A `before-after` is a two-column table; a `video` is its GIF, which GitHub renders from a raw URL, linking the WebM's page on `ci-screenshots`. Nothing is deleted from `ci-screenshots` yet (#40).
...
- No job saves to Actions' cache after PR code has run in it, because a `workflow_dispatch` run's cache scope is the dispatched ref, `develop` by default, which every branch reads. `request` caches nothing. `capture` only restores the Rust toolchain, which `ci.yml` saves, and caches npm only on `pull_request` runs, whose saves stay in the PR's own scope. The app zips are restore-only, as always.
...
The suite has these parts, in `ci/smoke/src/checks/`:
...
- `accounts.ts`: Sign in on a detected CLI opens a terminal running its login command (#115).
- `agents.ts`: a subagent end to end over the bundled wispd (#105). `ci/smoke/fixtures/fake-cli/claude`, first on `PATH`, answers wispd's detection and runs as a Claude Code worker: it writes a file in its worktree and echoes a follow-up. The check creates a project from a repository with a commit, runs **Wisp: Start Subagent**, sees the output in the subagent's tab, sees the Agents pill count it and the Agents panel list it as Needs review on this Mac, opens it again with Enter, and sends it a message from the composer. The steps live in `ci/screenshots/src/agents.ts`, which the `agents-window-agents-panel` and `agents-window-subagent` screenshots use too.
- `threads.ts`: a normal thread end to end over the bundled wispd (#110), with the same fake Claude Code. With a project open, **New Chat** opens upstream's new-session composer in the project's repository. The first message, after picking Claude Code, starts a thread that the sidebar lists under the repository in **Repositories**. Its chat shows the agent's work, with the agent's branch in the composer footer, and takes a follow-up from the composer. The steps live in `ci/screenshots/src/threads.ts`, which the `agents-window-new-chat` and `agents-window-repo-thread` screenshots use too.
...
- A PR skips `check-fork` and `test-wisp-browser` when its inputs already passed both:
  - The inputs are every tracked file under `editor/` (the pin, the patches, and anything #9 adds; `editor/vscode/` is never tracked), plus `.nvmrc`, `.gitattributes`, `scripts/editor/` (`test-wisp-browser` included), `check-fork`, and `ci.yml`.
  ...
  - Each pass saves an empty marker with `actions/cache` under that key, written only once both steps succeed. A PR can reuse a pass from `develop` or from an earlier push to the same PR.
  - Unlike a diff against the previous push, a later docs-only push cannot turn a failed check green.
  - The job always runs, so `ci` still gets its result. Pushes to `develop` and `main` always run both checks.
